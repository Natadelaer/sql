...
length(email) as "Длинa значения поля email", 
date(last_update) as "Дата последнего обновления записи"
from customer;
...
left(email, position('@' in email) - 1) as "email before @", 
...
select customer_id, email as "full email",
    initcap(left(email, position('@' in email) - 1)) as "email before @", 
    initcap(substring(email, position('@' in email) + 1)) as "email after @"
from customer;
